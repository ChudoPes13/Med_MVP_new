...
Frontend останавливает текущую озвучку при текстовом вводе или смене сессии. В backend у WebSocket есть событие `barge_in`, но оно не привязано к VAD/RMS и не вмешивается в `flush_audio`.

VAD/audio mechanics оставлены в рабочем варианте: микрофон отправляет PCM16, backend сам определяет конец фразы, а `Flush` только отправляет накопленный аудио-сегмент без отмены ответа. Если микрофон ловит звук колонок, для демо лучше использовать наушники или echo cancellation в браузере, что уже задано в UI.
