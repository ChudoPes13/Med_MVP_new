...
VAD/audio mechanics оставлены в рабочем варианте, который не завязан на прерывание:

- frontend отправляет PCM16 как раньше;
- backend последовательно обрабатывает VAD-сегмент и `flush_audio`;
- `Flush` только отправляет накопленный аудио-сегмент и не отменяет текущий ответ;
- frontend хранит активные TTS audio source и может остановить их при текстовом вводе или смене сессии;
- WebSocket event `barge_in` остается, но он не привязан к RMS/VAD, чтобы не ломать endpointing.
